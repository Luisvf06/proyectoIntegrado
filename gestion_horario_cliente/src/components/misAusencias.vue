<template>
  ...
  export default {
    ...
    methods: {
      ...
      getReadOnlyRow(ausencia) {
        const fecha = new Date(ausencia.fecha);
        return `
          <td class="border px-4 py-2">${fecha.getDate()}</td>
          <td class="border px-4 py-2">${fecha.toLocaleString('default', { month: 'long' })}</td>
      ...
      getEditableRow(ausencia) {
        const fecha = new Date(ausencia.fecha);
        return `
          <td class="border px-4 py-2"><input type="number" value="${fecha.getDate()}" data-field="fecha" min="1" max="31"></td>
          <td class="border px-4 py-2"><input type="text" value="${fecha.toLocaleString('default', { month: 'long' })}" data-field="mes"></td>
          <td class="border px-4 py-2"><input type="time" value="${ausencia.hora}" data-field="hora"></td>
          <td class="border px-4 py-2">${ausencia.id}</td>
          <td class="border px-4 py-2 flex flex-col space-y-2">
  ...
  const dia = row.querySelector('input[data-field="fecha"]').value;
  const mes = row.querySelector('input[data-field="mes"]').value;
  const hora = row.querySelector('input[data-field="hora"]').value;

  // Convertir el valor del mes a número de mes
  const monthNumber = new Date(Date.parse(mes + " 1, 2024")).getMonth();

  // Crear una nueva fecha con el año actual, mes y día seleccionado
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:8080/api/ausencias/${id}`, {
      method: 'PUT', // O 'PATCH' si prefieres hacer actualizaciones parciales
      headers: {
        'Authorization': `Bearer ${token}`,
